...
// Netlify Function proxy — server fetches image, browser gets it
const proxyImg = (url) =>
  url && url !== "N/A"
...
.mph-poster{position:absolute;bottom:-50px;left:24px;width:112px;height:168px;border-radius:12px;border:3px solid var(--s1);display:flex;align-items:center;justify-content:center;font-size:42px;font-weight:900;box-shadow:0 10px 36px rgba(0,0,0,.7);animation:float 4s ease-in-out infinite}
...
.wlbtn2{background:var(--s3);border:1.5px solid var(--bd);color:#ccc;border-radius:12px;padding:12px 18px;font-weight:600;cursor:pointer;font-size:13px;transition:all .2s;font-family:'Inter',sans-serif;white-space:nowrap}
  ...
  return (
    <>
            ...
            <>
              ...
              {hasMore && (
                <div className="smwrap">
                  <button className="smbtn" onClick={doLoadMore} disabled={loadingMore}>
        ...
        <div className="tover" onClick={() => setShowTrailer(null)}>
          <div className="tbox" onClick={e => e.stopPropagation()}>
            ...
            <div className="tframe">
              <iframe src={ytEmbed(showTrailer)} title="trailer" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen />
            </div>
          </div>
        </div>
      )}
      ...
      {selected && (() => {
        ...
        const pSrc = proxyImg(selected.Poster);
        return (
          <div className="mover" onClick={() => setSelected(null)}>
            <div className="mod" onClick={e => e.stopPropagation()}>
              <div className="mhero" style={{ background: `linear-gradient(135deg,${cols[0]},${cols[1]}30)` }}>
                {pSrc
                  ? <img className="mhbg" src={pSrc} alt="" onError={e => e.target.style.display = "none"} />
                  : <div className="mhph">🎬</div>
                }
                <div className="mhov" />
                {pSrc
                  ? <img className="mposter" src={pSrc} alt={selected.Title} onError={e => e.target.style.display = "none"} />
                  : <div className="mph-poster" style={{ background: `linear-gradient(135deg,${cols[0]},${cols[1]}40)`, color: cols[1] }}>{selected.Title?.[0]}</div>
                }
              </div>
              ...
              <div className="mbody">
                ...
                {selected.Plot && selected.Plot !== "N/A" && (
                  <>
                    <div className="mplot-label">📖 PLOT</div>
                ...
                <div className="macts">
                  <button className="btnr" onClick={() => { setShowTrailer(selected.Title); setSelected(null); }}>{t.watchTrailer}</button>
                  <button className="wlbtn2" style={{ background: "rgba(255,0,0,.1)", border: "1.5px solid rgba(255,0,0,.3)", color: "#ff6b6b" }} onClick={() => openYT(selected.Title)}>{t.watchYT}</button>
                  <button className={`wlbtn2 ${inWL ? "saved" : ""}`} onClick={() => toggleWL(selected)}>{inWL ? t.removeWL : t.addWL}</button>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="wlbox" onClick={e => e.stopPropagation()}>
            ...
            <div className="wlbody">
              {watchlist.length === 0 ? (
                ...
                const c = gCol(m.Genre); const ps = proxyImg(m.Poster);
                return (
                  <div key={m.imdbID} className="wlitem" onClick={() => { setSelected(m); setShowWL(false); }}>
                    {ps
                      ? <img className="wlpimg" src={ps} alt={m.Title} onError={e => e.target.style.display = "none"} />
